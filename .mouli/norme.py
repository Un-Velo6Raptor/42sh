    ...
    def check_header(self):
        if (self.nb_line == 1):
            ...
            if self.cheat:
                ...
                if test:
                    if not test.group(2) in 'Thomas GRANDJEAN':
            ...
            if self.cheat:
                ...
                test = re.search(p, self.line)
                if test:
                    if not test.group(1) in 'thomas.grandjean@epitech.eu':
...
##            self.check_operateur('+')
            self.check_typedef()
            self.check_regex('(= \w+([ \t]+[\*\+\-\/\|\&]\w+))|(= \w+([\*\+\-\/\|\&][ \t]+\w+))|(= \w+([\*\+\-\/\|\&]+\w+))', 'Opérateur mal placée')
            self.check_regex('[\[\(\=]([ \t]+)?[-+][ \t]+\w+', 'Espace en trop')
            self.check_regex('(([\[\(][ \t]+)|([ \t]+[\]\)]))', 'Espace avant/apres parentheses')
            self.check_regex('[ \t]?(if|else|return|while|for)[ \t]?\(+\w+ = ', 'Assignation dans condition')

    def print_error(self, msg):
        print("Erreur dans {} à la ligne {}:\n\033[0;31;40m {} \033[0;m").format(self.file, self.nb_line, msg)
        if self.printline:
            print("\033[0;33;40m {} \033[0;m").format(self.line)

    def cant_open(self, file):
        if (self.verbose or file == sys.argv[1]):
            print("Impossible d'ouvrir {}").format(file)
